      ...
      <div class="edit">
        <div class="input-group">
          ...
          <input
            ...
            @input="updateIdFromName(item.name)"
          />
        </div>
        ...
        <div class="input-group stretch">
          <label for="description"
            >Description
            <label class="muted">({{ item.description.length }} / 400) </label>
          </label>
          <textarea
            id="description"
...
const props = defineProps<{
  ...
  // The default is to edit an existing item. To create a new one, pass a function to onCreateNew.
  // All other props will be ignored in this case.
  onCreateNew?: (item: Item) => void;
}>();
...
const item = ref({ ...(props.item || blankItem) });
...
function onClickCreate() {
  if (!props.onCreateNew) return;
  props.onCreateNew(item.value);
  ModalController.close();
}

function onClickSave() {
  if (!props.onEdit) return;
  props.onEdit(item.value);
  ModalController.close();
}

function onClickDelete() {
  if (!props.onDelete) return;
  props.onDelete(item.value.id);
  ModalController.close();
}
</script>
